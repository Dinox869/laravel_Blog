<x-links>
    <section class="py-4 px-8 bg-gray-100 min-h-screen">
        <div class="justify-content-center ">
            <h1 class="font-bold text-xl text-center mb-10 ">UPDATE POST</h1>
            ...
            <div class="bg-white shadow-md rounded-xl px-8 pt-6 pd-8 mb-4 flex flex-col max-w-6xl mx-auto ">
                <form method="POST"  action="/update/{{$post->slug}}"  enctype="multipart/form-data" >
                @csrf
                <!-- title -->
                    <div class="mb-4">
                        <label for="title" class="block text-grey-darker text-sm font-bold mb-2">Title</label>
                        <input id="title" value="{{$post->title}}"  name="title" type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-grey-darker">
                        @error('title')
                    ...
                    <div class="mb-4">
                        ...
                        <div class="block flex justify-between">
                           <div class="w-4/5 ">
                               <input id="thumbnail" value="old('thumbnail', $post->thumbnail)" name="thumbnail" type="file" class="shadow appearance-none border rounded w-full py-2 px-3 text-grey-darker">
                           </div>
                            <img src="/storage/{{$post->thumbnail}}" height="140" width="140" class="rounded-xl">
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input id="excerpt"   value="{{$post->excerpt}}" name="excerpt" type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-grey-darker">
                        @error('excerpt')
                        <span class="text-sm text-red-500 font-italic mt-1">
                    {{ $message }}
                </span>
